...
class Model:
    ...
    def load_model(self, path, rank, deforum_models_path):
        if not os.path.exists(os.path.join(deforum_models_path,'RIFE43.pkl')):
            ...
            if checksum(os.path.join(deforum_models_path,'RIFE46.pkl')) != 'ed660f58708ee369a0b3855f64d2d07a6997d949f33067faae51d740123c5ee015901cc57553594f2df8ec08131a1c5f7c883c481eac0f9addd84379acea90c8':
        ...
        if rank <= 0:
            if torch.cuda.is_available():
                self.flownet.load_state_dict(convert(torch.load(os.path.join(deforum_models_path,'RIFE43.pkl').format(path))), False)
            else:
                self.flownet.load_state_dict(convert(torch.load(os.path.join(deforum_models_path,'RIFE43.pkl').format(path), map_location ='cpu')), False)
