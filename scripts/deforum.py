...
def run_deforum(*args, **kwargs):
    ...
    for i in range(times_to_run):
        ...
        args_loaded_ok, root, args, anim_args, video_args, parseq_args, loop_args, controlnet_args, glsl_args = deforum_args.process_args(args_dict, i)
...
def on_ui_tabs():
    with gr.Blocks(analytics_enabled=False) as deforum_interface:
        ...
        glsl_settings_component_list = [components[name] for name in deforum_args.glsl_args_names]
        stuff = gr.HTML("") # wrap gradio call garbage
        stuff.visible = False

        save_settings_btn.click(
            fn=wrap_gradio_call(deforum_settings.save_settings),
            inputs=[settings_path] + settings_component_list + video_settings_component_list + glsl_settings_component_list,
            outputs=[stuff],
        )
        ...
        load_video_settings_btn.click(
            ...
            inputs=[settings_path] + video_settings_component_list + glsl_settings_component_list,
            outputs=video_settings_component_list + [stuff] + glsl_settings_component_list,
        )
