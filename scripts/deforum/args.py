...
def DeforumAnimArgs():
    ...
    padding_mode = 'zeros'#@param ['border', 'reflection', 'zeros'] {type:'string'}
...
def setup_deforum_setting_dictionary(self, is_img2img, is_extension = True):
    ...
    with gr.Tab('Keyframes'):
        ...
        with gr.Row():
            ...
            border = gr.Dropdown(label="border", choices=['replicate', 'wrap', 'zeros'], value=da.border, type="value", elem_id="border", interactive=True)
        ...
        # Anti-blur
        i37 = gr.HTML("<p style=\"margin-bottom:0.75em\">Anti-blur settings</p>")
        with gr.Row():
        ...
        with gr.Row():
            force_color_with_webui = gr.Checkbox(label="force_color_with_webui", value=d.force_color_with_webui, interactive=True)
            inpaint_zeros = gr.Checkbox(label="inpaint_zeros", value=d.inpaint_zeros, interactive=True)
        with gr.Row():
            choice = mask_fill_choices[d.zeros_fill_mode]
            zeros_fill_mode = gr.Radio(label='zeros_fill_mode', choices=mask_fill_choices, value=choice, type="index")
        with gr.Row():
