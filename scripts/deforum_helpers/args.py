...
def DeforumArgs():
    ...
    overlay_mask = False  #@param {type:"boolean"}
    # Blur edges of final overlay mask, if used. Minimum = 0 (no blur)
    mask_overlay_blur = 0 #@param {type:"number"}

    #@markdown **Color Match Conditional Settings**
    colormatch_image = "https://www.saasdesign.io/wp-content/uploads/2021/02/palette-3-min-980x588.png" #@param {type:"string"}
    enable_colormatch_image = False #@param {type:"boolean"}

    fill = 1 #@param {type:"number"} 0-3
    full_res_mask = False #@param {type:"boolean"}
    full_res_mask_padding = 0 #@param {type:"number"}
...
def setup_deforum_setting_dictionary(self, is_img2img, is_extension = True):
    ...
    with gr.Tab('Keyframes'):
        ...
        with gr.Row():    
            colormatch_image = gr.Textbox(label="colormatch_image", lines=1, interactive=True, value = d.colormatch_image)
            enable_colormatch_image = gr.Checkbox(label="enable_colormatch_image", value=d.enable_colormatch_image, interactive=True)
